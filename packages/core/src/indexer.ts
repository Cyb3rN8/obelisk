...
interface BuildIndexOptions {
  ...
  // recovery, which needs the just-written transcript indexed cheaply, and by
  // an explicitly invoked `--build`, which must run now rather than answer
  // "recently built" (LOCAL: `--build` is incremental, so it no longer gets
  // the bypass for free via `force`).
  ignoreRecentBuild?: boolean;
